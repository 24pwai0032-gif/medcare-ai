...
import {
  ...
  HomeIcon, GridIcon, ClipboardIcon, LogOutIcon, ModuleIcon,
  CheckCircleIcon, ClockIcon, AlertTriangleIcon, SparklesIcon,
  SignalIcon, XIcon, ChevronRightIcon, LogoIcon,
...
interface PatientDashboardProps { user: any; onLogout: () => void; }

const modules = [
  { icon: LungsIcon,       t: 'X-Ray Analysis',  d: 'Chest X-ray, MRI & CT scan',  route: 'xray',         c: '#3B82F6', bg: 'rgba(59,130,246,.12)',  border: 'rgba(59,130,246,.25)' },
  { icon: BoneIcon,        t: 'Bone Scan',        d: 'Fracture & bone disease',      route: 'bone',         c: '#8B5CF6', bg: 'rgba(139,92,246,.12)',  border: 'rgba(139,92,246,.25)' },
  { icon: HeartPulseIcon,  t: 'ECG Analyzer',     d: 'Heart rhythm & cardiac',       route: 'ecg',          c: '#EF4444', bg: 'rgba(239,68,68,.12)',   border: 'rgba(239,68,68,.25)' },
  { icon: TestTubeIcon,    t: 'Blood Tests',      d: 'Full blood report analysis',   route: 'blood',        c: '#10B981', bg: 'rgba(16,185,129,.12)',  border: 'rgba(16,185,129,.25)' },
  { icon: BrainIcon,       t: 'Mental Health',    d: 'PHQ-9 & GAD-7 screening',      route: 'mental',       c: '#F59E0B', bg: 'rgba(245,158,11,.12)',  border: 'rgba(245,158,11,.25)' },
  { icon: StethoscopeIcon, t: 'Diagnosis AI',     d: 'Symptom-based diagnosis',      route: 'diagnosis',    c: '#06B6D4', bg: 'rgba(6,182,212,.12)',   border: 'rgba(6,182,212,.25)' },
  { icon: PillIcon,        t: 'Prescription',     d: 'Handwritten prescription',     route: 'prescription', c: '#EC4899', bg: 'rgba(236,72,153,.12)',  border: 'rgba(236,72,153,.25)' },
  { icon: ActivityIcon,    t: 'Vital Signs',      d: 'BP, sugar & oxygen tracking',  route: 'vitals',       c: '#14B8A6', bg: 'rgba(20,184,166,.12)',  border: 'rgba(20,184,166,.25)' },
  { icon: SirenIcon,       t: 'Emergency Aid',    d: 'Instant first aid guidance',   route: 'emergency',    c: '#F97316', bg: 'rgba(249,115,22,.12)',  border: 'rgba(249,115,22,.25)' },
];

const statusConfig: Record<string, { label: string; color: string; bg: string; Icon: React.FC<{size?:number}> }> = {
  pending:   { label: 'Pending',    color: '#FBBF24', bg: 'rgba(251,191,36,.12)',  Icon: ClockIcon },
  approved:  { label: 'Approved',   color: '#34D399', bg: 'rgba(52,211,153,.12)',  Icon: CheckCircleIcon },
  rejected:  { label: 'Attention',  color: '#F87171', bg: 'rgba(248,113,113,.12)', Icon: AlertTriangleIcon },
  completed: { label: 'Completed',  color: '#60A5FA', bg: 'rgba(96,165,250,.12)',  Icon: CheckCircleIcon },
};
...
export default function PatientDashboard({ user, onLogout }: PatientDashboardProps) {
  ...
  const stats = [
    ...
    { label: 'Approved',       value: scans.filter(s => s.status === 'approved').length,     Icon: CheckCircleIcon, c: '#34D399' },
    { label: 'Pending',        value: scans.filter(s => s.status === 'pending').length,      Icon: ClockIcon, c: '#FBBF24' },
    { label: 'AI Modules',     value: 9,                                                     Icon: SparklesIcon, c: '#A78BFA' },
  ];
  ...
  const navItems = [
    ...
    { id: 'modules', Icon: GridIcon,      label: 'Modules' },
    { id: 'scans',   Icon: ClipboardIcon, label: 'My Scans' },
  ];
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0B0F1A', fontFamily: "'DM Sans','Helvetica Neue',sans-serif", display: 'flex', overflow: 'hidden' }}>
      ...
      <aside className="sidebar" style={{ width: 228, flexShrink: 0, background: 'rgba(255,255,255,.03)', borderRight: '1px solid rgba(255,255,255,.06)', display: 'flex', flexDirection: 'column', position: 'relative', zIndex: 10, backdropFilter: 'blur(20px)' }}>
        <div style={{ padding: '20px 18px 16px', borderBottom: '1px solid rgba(255,255,255,.05)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: 'linear-gradient(135deg,#3B82F6,#8B5CF6)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(59,130,246,.4)', color: '#fff' }}>
              <LogoIcon size={20} />
            </div>
            <div>
        ...
        <div style={{ padding: '12px 10px 16px', borderTop: '1px solid rgba(255,255,255,.05)' }}>
          ...
          <button className="logout-btn" onClick={onLogout} style={{ width: '100%', background: 'rgba(248,113,113,.08)', border: '1px solid rgba(248,113,113,.18)', color: '#F87171', padding: '8px', borderRadius: 9, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
      ...
      <main style={{ flex: 1, overflow: 'auto', position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(11,15,26,.88)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,.05)', padding: '13px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(52,211,153,.08)', border: '1px solid rgba(52,211,153,.2)', borderRadius: 100, padding: '5px 11px' }}>
            <span style={{ color: '#34D399', display: 'flex' }}><SignalIcon size={13} /></span>
            ...
            <>
              <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 24 }}>
                {stats.map((s, i) => (
                  <div key={i} className="stat-card" style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 14, padding: '18px 16px' }}>
                    <div style={{ color: s.c, marginBottom: 8, display: 'flex' }}><s.Icon size={22} /></div>
              ...
              <div style={{ marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                  ...
                  <button onClick={() => setActiveTab('modules')} style={{ background: 'none', border: 'none', color: '#60A5FA', cursor: 'pointer', fontSize: 12.5, fontWeight: 600, fontFamily: 'inherit' }}>View all &rarr;</button>
                </div>
                <div className="modules-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 9 }}>
                  {modules.slice(0, 6).map((m, i) => (
                    <div key={i} className="mod-card" onClick={() => setActiveModule(m.route)}
                      ...
                      <div style={{ color: m.c, marginBottom: 7, display: 'flex', justifyContent: 'center' }}><m.icon size={28} /></div>
              ...
              <div className="modules-grid-full" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 10 }}>
                {modules.map((m, i) => (
                  <div key={i} className="mod-card" onClick={() => setActiveModule(m.route)}
                    ...
                    <div style={{ width: 48, height: 48, borderRadius: 13, flexShrink: 0, background: m.bg, border: `1px solid ${m.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: m.c }}>
                      <m.icon size={24} />
                    </div>
                    <div style={{ flex: 1 }}>
      ...
      {/* BOTTOM NAV (mobile) */}
      <div className="bottom-nav" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, zIndex: 100, background: 'rgba(11,15,26,.95)', borderTop: '1px solid rgba(255,255,255,.07)', backdropFilter: 'blur(20px)', padding: '8px 0 12px', justifyContent: 'space-around', alignItems: 'center' }}>
        {navItems.map(item => (
          <button key={item.id} onClick={() => setActiveTab(item.id as any)}
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: '6px 16px', borderRadius: 10, color: activeTab === item.id ? '#60A5FA' : 'rgba(255,255,255,.35)' }}>
            <item.Icon size={20} />
            <span style={{ fontSize: 11, fontWeight: 600 }}>{item.label}</span>
            {activeTab === item.id && <div style={{ width: 4, height: 4, borderRadius: '50%', background: '#3B82F6', marginTop: -2 }} />}
          </button>
        ))}
        <button onClick={onLogout} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', padding: '6px 16px', color: 'rgba(248,113,113,.7)' }}>
          <LogOutIcon size={20} />
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: '#0E1423', border: '1px solid rgba(255,255,255,.1)', borderRadius: 20, width: '100%', maxWidth: 540, maxHeight: '88vh', overflow: 'auto', boxShadow: '0 40px 100px rgba(0,0,0,.6)', animation: 'fadeUp .25s ease' }}>
            <div style={{ padding: '20px 22px 16px', borderBottom: '1px solid rgba(255,255,255,.06)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, background: '#0E1423', zIndex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 40, height: 40, borderRadius: 11, background: modules.find(m => m.route === selectedScan.scan_type)?.bg || 'rgba(255,255,255,.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: modules.find(m => m.route === selectedScan.scan_type)?.c || '#60A5FA' }}>
                  <ModuleIcon type={selectedScan.scan_type} size={20} />
                </div>
                <div>
              ...
              <button onClick={() => setSelectedScan(null)} style={{ width: 30, height: 30, borderRadius: '50%', background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.1)', cursor: 'pointer', color: 'rgba(255,255,255,.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <XIcon size={14} />
              </button>
            </div>
              ...
              {(() => {
                ...
                return (
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: st.bg, border: `1px solid ${st.color}30`, borderRadius: 100, padding: '5px 13px', marginBottom: 18 }}>
                    <span style={{ color: st.color, display: 'flex' }}><st.Icon size={13} /></span>
              ...
              {(selectedScan.severity || selectedScan.confidence) && (
                <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
                  {selectedScan.severity && (
                    <div style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 10, padding: '10px 14px', flex: 1 }}>
                      <div style={{ fontSize: 10, color: 'rgba(255,255,255,.3)', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 4 }}>Severity</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: '#E2E8F0' }}>{selectedScan.severity}</div>
                    </div>
                  )}
                  {selectedScan.confidence > 0 && (
                    <div style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 10, padding: '10px 14px', flex: 1 }}>
                      <div style={{ fontSize: 10, color: 'rgba(255,255,255,.3)', fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 4 }}>Confidence</div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: '#60A5FA' }}>{Math.round(selectedScan.confidence)}%</div>
                    </div>
                  )}
                </div>
              )}
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.07)', borderRadius: 12, padding: '14px 16px' }}>
                    <p style={{ color: 'rgba(255,255,255,.7)', fontSize: 13.5, lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>
                      {typeof selectedScan.report === 'string' ? selectedScan.report : JSON.stringify(selectedScan.report, null, 2)}
                    </p>
                  </div>
                </div>
              )}
      ...
      {items.map((scan, i) => {
        ...
        return (
          <div key={i} className="scan-row" onClick={() => onSelect(scan)}
            ...
            <div style={{ width: 36, height: 36, borderRadius: 9, background: 'rgba(255,255,255,.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,.4)', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: st.bg, border: `1px solid ${st.color}25`, borderRadius: 100, padding: '4px 11px', flexShrink: 0 }}>
              <span style={{ color: st.color, display: 'flex' }}><st.Icon size={11} /></span>
              <span style={{ fontSize: 11.5, color: st.color, fontWeight: 700 }}>{st.label}</span>
            </div>
            <span style={{ color: 'rgba(255,255,255,.2)', display: 'flex', flexShrink: 0 }}><ChevronRightIcon size={16} /></span>
          </div>
        );
      })}
    </div>
