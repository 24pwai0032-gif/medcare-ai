...
import {
  HeartPulseIcon, CheckCircleIcon, AlertTriangleIcon, ClockIcon,
  ShieldCheckIcon, SparklesIcon, SearchIcon, ClipboardIcon,
  RefreshIcon, PrinterIcon, ArrowLeftIcon, SignalIcon, DownloadIcon,
  DoctorIcon, LogoIcon, ActivityIcon,
      ...
      <div className="xw" style={{ minHeight: '100vh', background: '#060A14', fontFamily: "'Sora',sans-serif", color: '#fff', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 32px', borderBottom: '1px solid rgba(255,255,255,.06)', background: 'rgba(6,10,20,.85)', backdropFilter: 'blur(24px)', position: 'sticky', top: 0, zIndex: 50 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '44px', height: '44px', background: 'linear-gradient(135deg,#EF4444,#F87171)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(239,68,68,.3)', color: '#fff' }}>
          ...
          {step === 'upload' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1.1fr .9fr', gap: '20px' }}>
              <div>
                ...
                <div className="dz" onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) handleFile(f); }} onDragOver={e => e.preventDefault()} onDragLeave={() => {}} onClick={() => fileRef.current?.click()}
                  ...
                  {preview ? (
                    ...
                    <>
                      <div style={{ width: '72px', height: '72px', background: 'rgba(239,68,68,.08)', border: '1px solid rgba(239,68,68,.15)', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px', color: '#F87171' }}><HeartPulseIcon size={32} /></div>
                ...
                <button className="ab" disabled={!selectedFile} onClick={handleAnalyze}
                  style={{ width: '100%', padding: '16px', background: selectedFile ? 'linear-gradient(135deg,#EF4444,#F87171)' : 'rgba(255,255,255,.04)', color: selectedFile ? '#fff' : '#334155', borderRadius: '14px', fontSize: '15px', fontWeight: 700, fontFamily: 'Sora,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', cursor: selectedFile ? 'pointer' : 'not-allowed' }}>
                  <HeartPulseIcon size={16} /> Analyze ECG
                </button>
              </div>
              <div style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '20px', padding: '24px' }}>
                ...
                {[
                  { Icon: HeartPulseIcon,   title: 'Heart Rhythm',  desc: 'Sinus rhythm, arrhythmias, heart rate' },
                  { Icon: ActivityIcon,      title: 'Wave Morphology', desc: 'P-wave, QRS complex, T-wave analysis' },
                  { Icon: ClockIcon,         title: 'Intervals',     desc: 'PR, QT, QTc interval measurement' },
                  { Icon: SearchIcon,        title: 'ST Segment',    desc: 'Elevation, depression, ischemic changes' },
                  { Icon: SignalIcon,        title: 'Conduction',    desc: 'Bundle branch blocks, heart blocks' },
                ].map((item, i) => (
                  <div key={i} style={{ display: 'flex', gap: '12px', padding: '11px 0', borderBottom: i < 4 ? '1px solid rgba(255,255,255,.04)' : 'none' }}>
                    <div style={{ width: '34px', height: '34px', background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.15)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: '#F87171' }}><item.Icon size={16} /></div>
              ...
              <div style={{ maxWidth: '380px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {analysisSteps.map((s, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 16px', background: i <= currentStep ? 'rgba(239,68,68,.06)' : 'rgba(255,255,255,.02)', border: `1px solid ${i <= currentStep ? 'rgba(239,68,68,.15)' : 'rgba(255,255,255,.04)'}`, borderRadius: '10px', transition: 'all .3s' }}>
                    <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: i < currentStep ? '#10B981' : i === currentStep ? 'linear-gradient(135deg,#EF4444,#F87171)' : 'rgba(255,255,255,.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', flexShrink: 0, animation: i === currentStep ? 'spin 1s linear infinite' : 'none', color: '#fff' }}>{i < currentStep ? '✓' : i === currentStep ? '⟳' : ''}</div>
            ...
            <div>
              ...
              <div className="rs" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px', marginBottom: '20px' }}>
                ...
                <div style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '16px', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                  ...
                  <div style={{ marginBottom: '6px', color: '#F87171' }}><HeartPulseIcon size={26} /></div>
              ...
              <div className="rs" style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '16px', padding: '22px', marginBottom: '20px' }}>
                <div style={{ fontSize: '13px', fontWeight: 700, color: '#94A3B8', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}><ClipboardIcon size={14} /> AI Cardiology Report</div>
                {(() => {
                  ...
                  if (en.length > 1) return en.map((sec, si) => (
                    <div key={si} style={{ marginBottom: si < en.length - 1 ? '16px' : 0 }}>
                      {sec.title && <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'8px' }}><div style={{ width:'3px', height:'16px', borderRadius:'2px', background:'linear-gradient(180deg,#EF4444,#F87171)' }} /><span style={{ fontSize:'12px', fontWeight:800, color:'#FCA5A5', letterSpacing:'.04em', textTransform:'uppercase' }}>{sec.title.replace(/\*/g,'')}</span></div>}
                      <div style={{ paddingLeft: sec.title ? '11px' : 0, borderLeft: sec.title ? '1px solid rgba(255,255,255,.04)' : 'none' }}>
                        {sec.body.split('\n').map((l, li) => {
                          ...
                          if (isBullet) return <div key={li} style={{ display:'flex', alignItems:'flex-start', gap:'8px', padding:'3px 0' }}><div style={{ width:'5px', height:'5px', borderRadius:'50%', background:'#EF4444', marginTop:'6px', flexShrink:0 }} /><span style={{ fontSize:'13px', color:'#CBD5E1', lineHeight:1.7 }}>{t.replace(/^[-\u2022]\s*/, '').replace(/\*\*/g, '')}</span></div>;
              ...
              <div className="rs" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '12px' }}>
                <button className="rb" onClick={() => { setStep('upload'); setResult(null); setPreview(null); setSelectedFile(null); setError(''); }} style={{ background: 'linear-gradient(135deg,#EF4444,#F87171)', color: '#fff', padding: '14px', borderRadius: '12px', fontSize: '13px', fontWeight: 700, fontFamily: 'Sora,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', border: 'none', cursor: 'pointer' }}><RefreshIcon size={14} /> New Scan</button>
                <button className="rb" onClick={() => generateMedicalPDF({ scanType: 'ECG', report: result.report || '', urduReport: result.urdu_report || '', severity: result.severity || 'Normal', confidence: result.confidence ?? 0, time: result.time ?? result.time_seconds ?? 0, filename: selectedFile?.name || 'ecg.jpg', status: 'pending' })} style={{ background: 'linear-gradient(135deg,#10B981,#059669)', border: 'none', color: '#fff', padding: '14px', borderRadius: '12px', fontSize: '13px', fontWeight: 700, fontFamily: 'Sora,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', cursor: 'pointer' }}><DownloadIcon size={14} /> Download PDF</button>
