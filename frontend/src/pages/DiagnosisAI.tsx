...
import {
  BrainIcon, ArrowLeftIcon, AlertTriangleIcon, SearchIcon,
  ClipboardIcon, RefreshIcon,
...
const SYMPTOM_DB: Record<string, { conditions: string[]; severity: string; advice: string; urdu: string }> = {
  ...
  'chest pain': { conditions: ['Angina', 'Acid Reflux (GERD)', 'Muscle Strain', 'Anxiety'], severity: 'Severe', advice: 'SEEK IMMEDIATE MEDICAL ATTENTION. Call emergency services if accompanied by shortness of breath.', urdu: 'سینے میں درد — فوری طبی مدد حاصل کریں! سانس کی تکلیف ہو تو ایمرجنسی کال کریں۔' },
  ...
  'shortness of breath': { conditions: ['Asthma', 'Anxiety', 'Pneumonia', 'Heart Failure'], severity: 'Severe', advice: 'SEEK IMMEDIATE MEDICAL ATTENTION if severe or sudden onset. Use inhaler if prescribed.', urdu: 'سانس کی تکلیف — فوری طبی مدد حاصل کریں! اگر انہیلر ہے تو استعمال کریں۔' },
      ...
      <div className="xw" style={{ minHeight: '100vh', background: '#060A14', fontFamily: "'Sora',sans-serif", color: '#fff', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 32px', borderBottom: '1px solid rgba(255,255,255,.06)', background: 'rgba(6,10,20,.85)', backdropFilter: 'blur(24px)', position: 'sticky', top: 0, zIndex: 50 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '44px', height: '44px', background: 'linear-gradient(135deg,#3B82F6,#60A5FA)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(59,130,246,.3)', color: '#fff' }}><BrainIcon size={22} /></div>
            <div>
          ...
          {!result ? (
            <div>
              <div style={{ textAlign: 'center', marginBottom: '32px' }}>
                <div style={{ marginBottom: '16px', color: '#60A5FA' }}><BrainIcon size={48} /></div>
              ...
              <button className="ab" disabled={!symptoms.trim() || analyzing} onClick={handleAnalyze}
                ...
                {analyzing ? <><div style={{ width: '18px', height: '18px', border: '2px solid rgba(255,255,255,.3)', borderTop: '2px solid #fff', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /> Analyzing...</> : <><BrainIcon size={16} /> Analyze Symptoms</>}
              </button>

              <div style={{ marginTop: '24px', background: 'rgba(245,158,11,.04)', border: '1px solid rgba(245,158,11,.12)', borderRadius: '12px', padding: '14px 18px', fontSize: '12px', color: '#FCD34D', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ flexShrink: 0, display: 'flex' }}><AlertTriangleIcon size={14} /></span>Yeh tool sirf initial guidance ke liye hai — final diagnosis ke liye doctor se zaroor milen.
              </div>
            </div>
          ) : (
            <div>
              <div className="rs" style={{ display: 'flex', gap: '24px', background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '20px', padding: '24px', marginBottom: '20px' }}>
                <div style={{ width: '80px', height: '80px', background: 'rgba(59,130,246,.1)', border: '1px solid rgba(59,130,246,.2)', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: '#60A5FA' }}><BrainIcon size={36} /></div>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: getSev(result.severity).bg, border: `1px solid ${getSev(result.severity).border}`, color: getSev(result.severity).color, padding: '8px 20px', borderRadius: '20px', fontSize: '14px', fontWeight: 700 }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: getSev(result.severity).color, display: 'inline-block' }} />
                    {getSev(result.severity).label}
                  </div>
                </div>
              </div>
              ...
              <div className="rs" style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '16px', padding: '22px', marginBottom: '20px' }}>
                <div style={{ fontSize: '13px', fontWeight: 700, color: '#94A3B8', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}><SearchIcon size={14} /> Possible Conditions</div>
              ...
              <div className="rs" style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '16px', padding: '22px', marginBottom: '20px' }}>
                <div style={{ fontSize: '13px', fontWeight: 700, color: '#94A3B8', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}><ClipboardIcon size={14} /> Medical Advice</div>
              ...
              <div className="rs" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <button className="rb" onClick={() => { setResult(null); setSymptoms(''); }} style={{ background: 'linear-gradient(135deg,#3B82F6,#60A5FA)', color: '#fff', padding: '14px', borderRadius: '12px', fontSize: '13px', fontWeight: 700, fontFamily: 'Sora,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', border: 'none', cursor: 'pointer' }}><RefreshIcon size={14} /> New Check</button>
