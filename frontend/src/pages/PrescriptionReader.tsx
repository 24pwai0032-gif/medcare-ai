      ...
      <div className="xw" style={{ minHeight: '100vh', background: '#060A14', fontFamily: "'Sora',sans-serif", color: '#fff', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 32px', borderBottom: '1px solid rgba(255,255,255,.06)', background: 'rgba(6,10,20,.85)', backdropFilter: 'blur(24px)', position: 'sticky', top: 0, zIndex: 50 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '44px', height: '44px', background: 'linear-gradient(135deg,#A855F7,#C084FC)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(168,85,247,.3)', color: '#fff' }}>
              <PillIcon size={22} />
            </div>
            <div>
          ...
          {step === 'upload' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1.1fr .9fr', gap: '20px' }}>
              <div>
                ...
                <div className="dz" onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) handleFile(f); }} onDragOver={e => e.preventDefault()} onClick={() => fileRef.current?.click()}
                  ...
                  {preview ? (
                    ...
                    <>
                      <div style={{ width: '72px', height: '72px', background: 'rgba(168,85,247,.08)', border: '1px solid rgba(168,85,247,.15)', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px', color: '#C084FC' }}><PillIcon size={32} /></div>
                ...
                <button className="ab" disabled={!selectedFile} onClick={handleAnalyze}
                  style={{ width: '100%', padding: '16px', background: selectedFile ? 'linear-gradient(135deg,#A855F7,#C084FC)' : 'rgba(255,255,255,.04)', color: selectedFile ? '#fff' : '#334155', borderRadius: '14px', fontSize: '15px', fontWeight: 700, fontFamily: 'Sora,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', cursor: selectedFile ? 'pointer' : 'not-allowed' }}>
                  <PillIcon size={16} /> Read Prescription
                </button>
              </div>
              <div style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '20px', padding: '24px' }}>
                ...
                {[
                  { Icon: PillIcon,           title: 'Medicine Names',  desc: 'Handwritten & printed medications' },
                  { Icon: ClipboardIcon,       title: 'Dosage Details',  desc: 'Amount, frequency, duration' },
                  { Icon: ClockIcon,           title: 'Schedule',        desc: 'Morning/evening/night timing' },
                  { Icon: AlertTriangleIcon,   title: 'Warnings',        desc: 'Drug interactions & precautions' },
                  { Icon: FileTextIcon,        title: 'Instructions',    desc: 'Special instructions & notes' },
                ].map((item, i) => (
                  <div key={i} style={{ display: 'flex', gap: '12px', padding: '11px 0', borderBottom: i < 4 ? '1px solid rgba(255,255,255,.04)' : 'none' }}>
                    <div style={{ width: '34px', height: '34px', background: 'rgba(168,85,247,.1)', border: '1px solid rgba(168,85,247,.15)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: '#C084FC' }}><item.Icon size={16} /></div>
              ...
              <div style={{ maxWidth: '380px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {analysisSteps.map((s, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 16px', background: i <= currentStep ? 'rgba(168,85,247,.06)' : 'rgba(255,255,255,.02)', border: `1px solid ${i <= currentStep ? 'rgba(168,85,247,.15)' : 'rgba(255,255,255,.04)'}`, borderRadius: '10px', transition: 'all .3s' }}>
                    <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: i < currentStep ? '#10B981' : i === currentStep ? 'linear-gradient(135deg,#A855F7,#C084FC)' : 'rgba(255,255,255,.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', flexShrink: 0, animation: i === currentStep ? 'spin 1s linear infinite' : 'none', color: '#fff' }}>{i < currentStep ? '✓' : i === currentStep ? '⟳' : ''}</div>
            ...
            <div>
              ...
              <div className="rs" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px', marginBottom: '20px' }}>
                ...
                <div style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '16px', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                  ...
                  <div style={{ marginBottom: '6px', color: '#C084FC' }}><PillIcon size={26} /></div>
