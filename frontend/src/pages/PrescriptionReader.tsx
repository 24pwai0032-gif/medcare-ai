...
const PrescriptionReader = ({ onBack }: { onBack: () => void }) => {
  ...
  const analysisSteps = ['Image uploading...', 'OCR processing...', 'AI reading prescription...', 'Identifying medications...', 'Finalizing results...'];
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE}/analyze/prescription`, { method: 'POST', headers: { 'Authorization': `Bearer ${getToken()}` }, body: fd });
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @keyframes glowProg{0%,100%{box-shadow:0 0 8px rgba(168,85,247,.4)}50%{box-shadow:0 0 22px rgba(168,85,247,.6)}}
    .xw{animation:fadeUp .4s ease}.dz{transition:all .3s;cursor:pointer}
    .dz:hover{border-color:rgba(168,85,247,.4)!important;background:rgba(168,85,247,.04)!important}
  ...
  return (
    <>
      ...
      <div className="xw" style={{ minHeight: '100vh', background: '#060A14', fontFamily: "'Sora',sans-serif", color: '#fff', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'fixed', top: '-250px', right: '-200px', width: '700px', height: '700px', background: 'radial-gradient(circle,rgba(168,85,247,.1) 0%,transparent 70%)', borderRadius: '50%', animation: 'blobFloat 12s ease infinite', pointerEvents: 'none' }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 32px', borderBottom: '1px solid rgba(255,255,255,.06)', background: 'rgba(6,10,20,.85)', backdropFilter: 'blur(24px)', position: 'sticky', top: 0, zIndex: 50 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '44px', height: '44px', background: 'linear-gradient(135deg,#A855F7,#C084FC)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px', boxShadow: '0 4px 20px rgba(168,85,247,.3)' }}>💊</div>
            <div>
              <div style={{ fontSize: '17px', fontWeight: 800, color: '#F1F5F9' }}>Prescription Reader</div>
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '32px 24px' }}>
          {step === 'upload' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1.1fr .9fr', gap: '20px' }}>
              <div>
                ...
                <div className="dz" onDrop={e => { e.preventDefault(); const f = e.dataTransfer.files[0]; if (f) handleFile(f); }} onDragOver={e => e.preventDefault()} onClick={() => fileRef.current?.click()}
                  ...
                  {preview ? (
                    <>
                      <img src={preview} alt="rx" style={{ width: '100%', maxWidth: '260px', maxHeight: '200px', objectFit: 'contain', borderRadius: '12px' }} />
                    ...
                    <>
                      <div style={{ width: '72px', height: '72px', background: 'rgba(168,85,247,.08)', border: '1px solid rgba(168,85,247,.15)', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '32px', marginBottom: '16px' }}>💊</div>
                      <div style={{ fontSize: '15px', fontWeight: 700, color: '#CBD5E1', marginBottom: '8px' }}>Prescription Image Drop Karo</div>
                ...
                <button className="ab" disabled={!selectedFile} onClick={handleAnalyze}
                  style={{ width: '100%', padding: '16px', background: selectedFile ? 'linear-gradient(135deg,#A855F7,#C084FC)' : 'rgba(255,255,255,.04)', color: selectedFile ? '#fff' : '#334155', borderRadius: '14px', fontSize: '15px', fontWeight: 700, fontFamily: 'Sora,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
                  💊 Read Prescription
                </button>
              </div>
              <div style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '20px', padding: '24px' }}>
                ...
                {[{ icon: '💊', title: 'Medicine Names', desc: 'Handwritten & printed medications' }, { icon: '📋', title: 'Dosage Details', desc: 'Amount, frequency, duration' }, { icon: '⏰', title: 'Schedule', desc: 'Morning/evening/night timing' }, { icon: '⚠️', title: 'Warnings', desc: 'Drug interactions & precautions' }, { icon: '🗒️', title: 'Instructions', desc: 'Special instructions & notes' }].map((item, i) => (
                  <div key={i} style={{ display: 'flex', gap: '12px', padding: '11px 0', borderBottom: i < 4 ? '1px solid rgba(255,255,255,.04)' : 'none' }}>
                    <div style={{ width: '34px', height: '34px', background: 'rgba(168,85,247,.1)', border: '1px solid rgba(168,85,247,.15)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', flexShrink: 0 }}>{item.icon}</div>
          ...
          {step === 'analyzing' && (
            <div style={{ textAlign: 'center', padding: '60px 24px' }}>
              <div style={{ position: 'relative', width: '220px', height: '220px', margin: '0 auto 40px', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 0 60px rgba(168,85,247,.2)' }}>
                {preview && <img src={preview} alt="rx" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(30%) contrast(1.1)' }} />}
                <div style={{ position: 'absolute', left: 0, right: 0, height: '3px', background: 'linear-gradient(90deg,transparent,#A855F7,#C084FC,transparent)', animation: 'scanLine 1.8s linear infinite', boxShadow: '0 0 20px rgba(168,85,247,.8)' }} />
              </div>
              <h2 style={{ fontSize: '26px', fontWeight: 800, marginBottom: '8px', background: 'linear-gradient(135deg,#C084FC,#60A5FA)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>AI Reading Prescription...</h2>
              <p style={{ color: '#334155', marginBottom: '32px', fontSize: '14px' }}>AI model prescription parh raha hai</p>
              <div style={{ maxWidth: '420px', margin: '0 auto 28px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}><span style={{ fontSize: '12px', color: '#475569' }}>Progress</span><span style={{ fontSize: '12px', color: '#C084FC', fontWeight: 700 }}>{Math.round(progress)}%</span></div>
                <div style={{ height: '8px', background: 'rgba(255,255,255,.05)', borderRadius: '4px', overflow: 'hidden' }}><div style={{ height: '100%', width: `${progress}%`, background: 'linear-gradient(90deg,#A855F7,#C084FC)', borderRadius: '4px', transition: 'width .4s ease', animation: 'glowProg 2s ease infinite' }} /></div>
              </div>
              <div style={{ maxWidth: '380px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {analysisSteps.map((s, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 16px', background: i <= currentStep ? 'rgba(168,85,247,.06)' : 'rgba(255,255,255,.02)', border: `1px solid ${i <= currentStep ? 'rgba(168,85,247,.15)' : 'rgba(255,255,255,.04)'}`, borderRadius: '10px', transition: 'all .3s' }}>
                    <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: i < currentStep ? '#10B981' : i === currentStep ? 'linear-gradient(135deg,#A855F7,#C084FC)' : 'rgba(255,255,255,.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', flexShrink: 0, animation: i === currentStep ? 'spin 1s linear infinite' : 'none' }}>{i < currentStep ? '✓' : i === currentStep ? '⟳' : ''}</div>
          ...
          {step === 'result' && result && sev && (
            <div>
              ...
              <div className="rs" style={{ display: 'flex', gap: '24px', background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '20px', padding: '24px', marginBottom: '20px' }}>
                {preview && <img src={preview} alt="rx" style={{ width: '130px', height: '130px', objectFit: 'cover', borderRadius: '14px', flexShrink: 0 }} />}
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '11px', color: '#475569', marginBottom: '6px', fontWeight: 600, letterSpacing: '.05em', textTransform: 'uppercase' }}>Prescription — Reading Complete</div>
                  <h2 style={{ fontSize: '22px', fontWeight: 800, color: '#F1F5F9', marginBottom: '14px' }}>AI Prescription Report</h2>
              ...
              <div className="rs" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px', marginBottom: '20px' }}>
                ...
                <div style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '16px', padding: '20px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  ...
                  <div style={{ fontSize: '14px', fontWeight: 700, color: '#C084FC' }}>Prescription</div>
                </div>
              </div>
              ...
              <div className="rs" style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '16px', padding: '22px', marginBottom: '20px' }}>
                <div style={{ fontSize: '13px', fontWeight: 700, color: '#94A3B8', marginBottom: '14px' }}>📋 AI Prescription Report</div>
                <div style={{ fontSize: '14px', color: '#CBD5E1', lineHeight: 1.9, whiteSpace: 'pre-line' }}>{result.report || '—'}</div>
              </div>

              {result.urdu_report && (
                <div className="rs" style={{ background: 'rgba(168,85,247,.04)', border: '1px solid rgba(168,85,247,.12)', borderRadius: '16px', padding: '22px', marginBottom: '20px', direction: 'rtl' }}>
                  <div style={{ fontSize: '13px', fontWeight: 700, color: '#C084FC', marginBottom: '14px' }}>🇵🇰 اردو رپورٹ</div>
              ...
              <div className="rs" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                <button className="rb" onClick={() => { setStep('upload'); setResult(null); setPreview(null); setSelectedFile(null); setError(''); }} style={{ background: 'linear-gradient(135deg,#A855F7,#C084FC)', color: '#fff', padding: '14px', borderRadius: '12px', fontSize: '13px', fontWeight: 700, fontFamily: 'Sora,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', border: 'none' }}>🔄 New Scan</button>
