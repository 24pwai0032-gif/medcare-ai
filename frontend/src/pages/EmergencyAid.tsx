...
import {
  SirenIcon, HeartPulseIcon, LungsIcon, AlertTriangleIcon,
  MedicalCrossIcon, BoneIcon, SignalIcon, ArrowLeftIcon,
  ClipboardIcon,
} from '../components/Icons';

type EmergencyEntry = {
  id: string; Icon: React.FC<{ size?: number }>; title: string; titleUr: string;
  symptoms: string[]; steps: string[]; stepsUr: string[];
};

const EMERGENCIES: EmergencyEntry[] = [
  { id: 'heart-attack', Icon: HeartPulseIcon, title: 'Heart Attack', titleUr: 'دل کا دورہ',
  ...
  { id: 'choking', Icon: LungsIcon, title: 'Choking', titleUr: 'گلا بند ہونا',
  ...
  { id: 'burns', Icon: AlertTriangleIcon, title: 'Burns', titleUr: 'جلنا',
  ...
  { id: 'bleeding', Icon: MedicalCrossIcon, title: 'Severe Bleeding', titleUr: 'شدید خون بہنا',
  ...
  { id: 'fracture', Icon: BoneIcon, title: 'Fracture', titleUr: 'ہڈی ٹوٹنا',
  ...
  { id: 'seizure', Icon: SignalIcon, title: 'Seizure', titleUr: 'مرگی کا دورہ',
    ...
    <>
      ...
      <div className="xw" style={{ minHeight: '100vh', background: '#060A14', fontFamily: "'Sora',sans-serif", color: '#fff', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 32px', borderBottom: '1px solid rgba(255,255,255,.06)', background: 'rgba(6,10,20,.85)', backdropFilter: 'blur(24px)', position: 'sticky', top: 0, zIndex: 50 }}>
          <button className="bk" onClick={selected ? () => setSelected(null) : onBack} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.08)', color: '#64748B', padding: '8px 18px', borderRadius: '10px', fontSize: '13px', fontFamily: 'Sora,sans-serif', cursor: 'pointer' }}>
            <ArrowLeftIcon size={14} /> Back
          </button>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ width: '44px', height: '44px', background: 'linear-gradient(135deg,#EF4444,#F87171)', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 20px rgba(239,68,68,.3)', animation: 'pulseRed 2s infinite', color: '#fff' }}><SirenIcon size={22} /></div>
            <div>
          ...
          {!selected ? (
            <div>
              <div style={{ textAlign: 'center', marginBottom: '32px' }}>
                <div style={{ marginBottom: '16px', color: '#F87171' }}><SirenIcon size={48} /></div>
              ...
              <div style={{ background: 'rgba(239,68,68,.06)', border: '1px solid rgba(239,68,68,.2)', borderRadius: '14px', padding: '16px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: 44, height: 44, background: 'rgba(239,68,68,.12)', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: '#F87171' }}><SirenIcon size={22} /></div>
                <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px' }}>
                {EMERGENCIES.map(e => (
                  <div key={e.id} className="ec" onClick={() => setSelected(e.id)}
                    ...
                    <div style={{ marginBottom: '12px', color: '#F87171', display: 'flex', justifyContent: 'center' }}><e.Icon size={36} /></div>
            ...
            <div>
              <div className="rs" style={{ display: 'flex', gap: '24px', background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '20px', padding: '24px', marginBottom: '20px' }}>
                <div style={{ width: '80px', height: '80px', background: 'rgba(239,68,68,.1)', border: '1px solid rgba(239,68,68,.2)', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, animation: 'pulseRed 2s infinite', color: '#F87171' }}><emergency.Icon size={36} /></div>
                <div style={{ flex: 1 }}>
              ...
              <div className="rs" style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '16px', padding: '22px', marginBottom: '20px' }}>
                <div style={{ fontSize: '13px', fontWeight: 700, color: '#FCA5A5', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}><AlertTriangleIcon size={14} /> Signs &amp; Symptoms</div>
              ...
              <div className="rs" style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '16px', padding: '22px', marginBottom: '20px' }}>
                <div style={{ fontSize: '13px', fontWeight: 700, color: '#94A3B8', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}><ClipboardIcon size={14} /> Step-by-Step First Aid</div>
              ...
              <div className="rs" style={{ background: 'rgba(245,158,11,.04)', border: '1px solid rgba(245,158,11,.12)', borderRadius: '12px', padding: '14px 18px', fontSize: '12px', color: '#FCD34D', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span style={{ flexShrink: 0, display: 'flex' }}><AlertTriangleIcon size={14} /></span>Yeh guide basic first aid ke liye hai — professional medical help ki jagah nahi le sakti.
              </div>

              <div className="rs" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <button className="rb" onClick={() => setSelected(null)} style={{ background: 'linear-gradient(135deg,#EF4444,#F87171)', color: '#fff', padding: '14px', borderRadius: '12px', fontSize: '13px', fontWeight: 700, fontFamily: 'Sora,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', border: 'none', cursor: 'pointer' }}><SirenIcon size={14} /> Other Emergencies</button>
