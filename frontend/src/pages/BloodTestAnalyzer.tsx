...
import {
  ...
  RefreshIcon, PrinterIcon, ArrowLeftIcon, MicroscopeIcon, DownloadIcon,
...
const BloodTestAnalyzer = ({ onBack }: { onBack: () => void }) => {
  ...
  const analysisSteps = ['Image uploading...', 'Preprocessing report...', 'AI analyzing blood values...', 'Generating report...', 'Finalizing results...'];
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const res = await fetch(`${BASE}/analyze/blood-test`, { method: 'POST', headers: { 'Authorization': `Bearer ${getToken()}` }, body: fd });
  ...
  const css = `
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @keyframes glowProg{0%,100%{box-shadow:0 0 8px rgba(16,185,129,.4)}50%{box-shadow:0 0 22px rgba(16,185,129,.6)}}
    .xw{animation:fadeUp .4s ease}.dz{transition:all .3s;cursor:pointer}
    .dz:hover{border-color:rgba(16,185,129,.4)!important;background:rgba(16,185,129,.04)!important}
  ...
  return (
    <>
      ...
      <div className="xw" style={{ minHeight: '100vh', background: '#060A14', fontFamily: "'Sora',sans-serif", color: '#fff', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'fixed', top: '-250px', right: '-200px', width: '700px', height: '700px', background: 'radial-gradient(circle,rgba(16,185,129,.1) 0%,transparent 70%)', borderRadius: '50%', animation: 'blobFloat 12s ease infinite', pointerEvents: 'none' }} />
          ...
          {step === 'analyzing' && (
            <div style={{ textAlign: 'center', padding: '60px 24px' }}>
              <div style={{ position: 'relative', width: '220px', height: '220px', margin: '0 auto 40px', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 0 60px rgba(16,185,129,.2)' }}>
                {preview && <img src={preview} alt="blood" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'grayscale(30%) contrast(1.1)' }} />}
                <div style={{ position: 'absolute', left: 0, right: 0, height: '3px', background: 'linear-gradient(90deg,transparent,#10B981,#34D399,transparent)', animation: 'scanLine 1.8s linear infinite', boxShadow: '0 0 20px rgba(16,185,129,.8)' }} />
              </div>
              <h2 style={{ fontSize: '26px', fontWeight: 800, marginBottom: '8px', background: 'linear-gradient(135deg,#34D399,#60A5FA)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>AI Analysis Chal Rahi Hai...</h2>
              <p style={{ color: '#334155', marginBottom: '32px', fontSize: '14px' }}>AI model blood values analyze kar raha hai</p>
              <div style={{ maxWidth: '420px', margin: '0 auto 28px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}><span style={{ fontSize: '12px', color: '#475569' }}>Progress</span><span style={{ fontSize: '12px', color: '#34D399', fontWeight: 700 }}>{Math.round(progress)}%</span></div>
                <div style={{ height: '8px', background: 'rgba(255,255,255,.05)', borderRadius: '4px', overflow: 'hidden' }}><div style={{ height: '100%', width: `${progress}%`, background: 'linear-gradient(90deg,#10B981,#34D399)', borderRadius: '4px', transition: 'width .4s ease', animation: 'glowProg 2s ease infinite' }} /></div>
              </div>
          ...
          {step === 'result' && result && sev && (
            <div>
              ...
              <div className="rs" style={{ display: 'flex', gap: '24px', background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.06)', borderRadius: '20px', padding: '24px', marginBottom: '20px' }}>
                {preview && <img src={preview} alt="blood" style={{ width: '130px', height: '130px', objectFit: 'cover', borderRadius: '14px', flexShrink: 0 }} />}
                <div style={{ flex: 1 }}>
                ...
                {(() => {
                  ...
                  if (en.length > 1) return en.map((sec, si) => (
                    <div key={si} style={{ marginBottom: si < en.length - 1 ? '16px' : 0 }}>
                      {sec.title && <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'8px' }}><div style={{ width:'3px', height:'16px', borderRadius:'2px', background:'linear-gradient(180deg,#10B981,#34D399)' }} /><span style={{ fontSize:'12px', fontWeight:800, color:'#6EE7B7', letterSpacing:'.04em', textTransform:'uppercase' }}>{sec.title.replace(/\*/g,'')}</span></div>}
                      <div style={{ paddingLeft: sec.title ? '11px' : 0, borderLeft: sec.title ? '1px solid rgba(255,255,255,.04)' : 'none' }}>
                        {sec.body.split('\n').map((l, li) => {
                          ...
                          if (isBullet) return <div key={li} style={{ display:'flex', alignItems:'flex-start', gap:'8px', padding:'3px 0' }}><div style={{ width:'5px', height:'5px', borderRadius:'50%', background:'#10B981', marginTop:'6px', flexShrink:0 }} /><span style={{ fontSize:'13px', color:'#CBD5E1', lineHeight:1.7 }}>{t.replace(/^[-\u2022]\s*/, '').replace(/\*\*/g, '')}</span></div>;
              ...
              <div className="rs" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '12px' }}>
                ...
                <button className="rb" onClick={() => generateMedicalPDF({ scanType: 'Blood Test', report: result.report || '', urduReport: result.urdu_report || '', severity: result.severity || 'Normal', confidence: result.confidence ?? 0, time: result.time ?? result.time_seconds ?? 0, filename: selectedFile?.name || 'blood.jpg', status: 'pending' })} style={{ background: 'linear-gradient(135deg,#3B82F6,#2563EB)', border: 'none', color: '#fff', padding: '14px', borderRadius: '12px', fontSize: '13px', fontWeight: 700, fontFamily: 'Sora,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', cursor: 'pointer' }}><DownloadIcon size={14} /> Download PDF</button>
